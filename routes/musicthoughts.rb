...
class MusicThoughts < Sinatra::Base
	...
	before do
		...
		#@mt = B50D::MusicThoughts.new('live')
		#@rand1 = @mt.thought_random(@lang)
	end
	...
	get %r{^/contributor/([0-9]+)} do |id|
		...
		#@contributor = @mt.contributor(@lang, id)
	...
	get '/search' do
		...
		if params[:q]
			...
			#@results = @mt.search(@lang, @searchterm)
		end
		erb :search
	end
